...
birthDataQueryString :: BirthData -> Text
birthDataQueryString BirthData {..} =
  "?" <>
  "location=" <> (birthLocation & locationInput) <>
  "&day=" <> (birthLocalTime & fmt "%-d") <>
  "&month=" <> (birthLocalTime & fmt "%-m") <>
  "&year=" <> (birthLocalTime & fmt "%Y") <>
  "&hour=" <> (birthLocalTime & fmt "%l") <>
  "&minute=" <> (birthLocalTime & fmt "%-M") <>
  "&day-part=" <> (birthLocalTime & fmt "%P") <>
  "&lat=" <> (birthLocation & locationLatitude & show & pack) <>
  "&lng=" <> (birthLocation & locationLongitude & show & pack)
  where
    fmt s = pack . (formatTime defaultTimeLocale s)

natalChartLink :: BirthData -> Text
natalChartLink = ("/full-chart" <>) . birthDataQueryString
